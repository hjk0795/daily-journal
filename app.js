...
app.get("/", function(req, res) {
  res.render(__dirname+"/views/home.ejs", {Content: homeStartingContent, post: Post.find()});
})
...
app.get("/posts/:anyword", function(req, res) {
  Post.find().forEach(function(post) {
    if (_.lowerCase(req.params.anyword) == _.lowerCase(post.title)) {
      console.log("Match!");

      res.render(__dirname+"/views/post.ejs", {postTitle: post.title, postContent: post.content});
    }
  })
})
